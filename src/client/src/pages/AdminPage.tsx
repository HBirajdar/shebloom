// @ts-nocheck
// Bug B fix: AdminPage now uses backend API instead of useAyurvedaStore
...
interface AdminArticle {
  ...
  emoji: string; isPublished: boolean; isFeatured: boolean; targetAudience: TargetAudience[]; createdAt: string;
}
...
export default function AdminPage() {
  ...
  // Bug B fix: data state from backend API
  ...
  // Bug B fix: on unlock → fetch dashboard from backend
  ...
  // Bug B fix: all CRUD via backend API
  ...
  const handleAddArticle = async () => {
    ...
    try {
      const res = await adminAPI.createArticle({ ...na, author: 'chief' });
