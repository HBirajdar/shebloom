...
const StarRating = ({ rating }: { rating: number }) => (
  <div className="flex items-center gap-0.5">
    {[1,2,3,4,5].map(i => <span key={i} className="text-[10px]" style={{ color: i <= Math.round(rating) ? '#F59E0B' : '#D1D5DB' }}>{'\u2605'}</span>)}
      ...
      <div className="px-5 pt-4 space-y-4">
        ...
        {view === 'shop' && (<>
          <div className="rounded-2xl p-5 text-white relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #065F46, #059669, #10B981)' }}>
            ...
            <p className="text-xs text-white/80 mt-1 max-w-[220px]">Every product freshly prepared by {chief?.name || 'our doctor'} using pure organic herbs.</p>
          ...
          <div>
            <h3 className="text-xs font-bold text-gray-400 uppercase mb-2">{cat === 'all' ? 'All Products' : catLabels[cat].label} ({visibleProducts.length})</h3>
            {visibleProducts.map(p => (
              <button key={p.id} onClick={() => setSelProduct(p)} className="w-full bg-white rounded-2xl p-3 shadow-sm flex gap-3 text-left active:scale-[0.98] transition-transform mb-3">
        ...
        {/* DOCTOR - Fixed to match DoctorListing interface */}
        {view === 'doctor' && chief && (<>
          <div className="bg-white rounded-3xl p-5 shadow-sm text-center">
            ...
            <span className="inline-block mt-3 text-[9px] font-bold bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full">{'\u{1F451}'} Chief Doctor \u2022 SheBloom</span>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
            <h3 className="text-xs font-bold text-gray-400 uppercase">Details</h3>
            {[
              ...
              { l: 'Qualification', v: chief.qualification, e: '\u{1F393}' },
            ].map(r => (
          ...
          <div className="bg-white w-full max-w-[430px] rounded-t-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1"><StarRating rating={selProduct.rating} /><span className="text-[10px] text-gray-400">({selProduct.reviews} reviews)</span></div>
              ...
              <button className="w-full py-3.5 rounded-2xl text-white font-bold text-sm active:scale-95" style={{ background: 'linear-gradient(135deg, #059669, #10B981)' }}>{'\u{1F6D2}'} Add to Cart \u2022 {'\u20B9'}{selProduct.discountPrice || selProduct.price}</button>
            </div>
          </div>
        </div>
      )}
