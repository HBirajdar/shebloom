...
export const authenticate = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    let user = await cacheGet<any>(`user:${decoded.userId}:basic`);
    if (!user) {
      ...
      user = dbUser;
      await cacheSet(`user:${decoded.userId}:basic`, user, 300);
    }
    ...
    if (error instanceof jwt.JsonWebTokenError) { res.status(401).json({ success: false, error: 'Invalid token' }); return; }
...
export const optionalAuth = async (req: AuthRequest, _res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (h?.startsWith('Bearer ')) {
      const d = jwt.verify(h.slice(7), process.env.JWT_SECRET!) as { userId: string; role: string };
      req.user = { id: d.userId, role: d.role };
    }
  } catch {}
  next();
};
